...
def d4(r, c, n=None, m=None):
    return [(i, j) for i, j in [(r, c+1), (r-1, c), (r, c-1), (r+1, c)]
            if m is None or 0 <= i < n and 0 <= j < m]

def d8(r, c, n=None, m=None):
    return [(i, j) for i in [r-1, r, r+1] for j in [c-1, c, c+1]
            if (i, j) != (r, c) and (m is None or 0 <= i < n and 0 <= j < m)]