/**
 * Author: Joseph Jackson, benq
 * Date: 2024-07-04
 * License: CC0
 * Time: O(nlogn)
 * Description: Suffix array + LCP between two suffixes
 * Usage: SuffixArray sa(str);  // ignore sa.sa[0]
 *        for (int i = 1; i <= str.size(); ++i) cout << str.substr(sa.sa[i]) << endl;
 *        str.lcp(0, 1) == lcp(str[0:], str[1:])
 * Status: tested
 */
